...
public class BlogSql {
    ...
    public static String buildQueryBlogListSQL(Map<String, Object> params) {
        ...
        sql += where(params);
        sql += " GROUP BY b.blog_id ORDER BY b.update_time DESC ";
        return sql;
    }
    ...
    public static final String GROUP_BY_MONTHLY = "SELECT DATE_FORMAT(update_time, '%Y-%m') as month, COUNT(1) as count FROM t_blog WHERE if_pub = 1 " +
            "GROUP BY DATE_FORMAT(update_time, '%Y-%m') ORDER BY month DESC";
    ...
    private static String where(Map<String, Object> params) {
        ...
        if (!StringUtils.isEmpty(params.get("month"))) {
            whereSql += " AND date_format(update_time, '%Y-%m') = :month ";
        }
